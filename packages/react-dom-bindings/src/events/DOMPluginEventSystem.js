...
export const nonDelegatedEvents: Set<DOMEventName> = new Set([
  ...
  /** @foxzero-007 事件机制 媒体相关事件react也不处理 */
  ...mediaEventTypes,
]);
...
export function listenToNativeEventForNonManagedEventTarget(
  ...
  /** @foxzero-007 事件机制 这里看起来是用来判断当前浏览器的事件机制，冒泡/捕获 */
  if (!listenerSet.has(listenerSetKey)) {
    if (isCapturePhaseListener) {
      eventSystemFlags |= IS_CAPTURE_PHASE;
    }
...
export function listenToAllSupportedEvents(rootContainerElement: EventTarget) {
  if (!(rootContainerElement: any)[listeningMarker]) {
    ...
    allNativeEvents.forEach(domEventName => {
      ...
      /**
       * @foxzero-007 事件机制 对‘selectionchange’ 单独处理了 因为不冒泡且必须绑定在dom上
       * 其余事件可以往rootContainerElement上面挂
       */
      if (domEventName !== 'selectionchange') {
        if (!nonDelegatedEvents.has(domEventName)) {
          /** @foxzero-007 事件机制 如果不是不需要处理的事件 */
          listenToNativeEvent(domEventName, false, rootContainerElement);
        }
        /** @foxzero-007 事件机制 如果是不需要处理的事件 */
        listenToNativeEvent(domEventName, true, rootContainerElement);
      }
    });
    /** @foxzero-007 事件机制 获取顶层的dom对象？ */
    const ownerDocument =
    ...
    if (ownerDocument !== null) {
      ...
      /** @foxzero-007 事件机制 这里应该是单独处理了selectionChange事件 因为需要绑定在dom上 */
...
/**
 *
 * @param {*} targetContainer 目标container （Dom或者reactRootContainer）
 * @param {*} domEventName 事件名称
 * @param {*} eventSystemFlags 事件机制（捕获/冒泡）
 * @param {*} isCapturePhaseListener 是否需要处理事件（入口处的筛选 listenToAllSupportedEvents）
...
function addTrappedEventListener(
  ...
  /** @foxzero-007 事件机制 如果支持passive并且事件为滚动 */
  ...
  /** @foxzero-007 事件机制 获取目标容器？ */
  targetContainer =
  /** TODO:  @foxzero-007 事件机制 enableLegacyFBSupport 某个标识*/
  ...
  /** @foxzero-007 事件机制 在某种情况下（这个某种情况看起来是指一个一次性事件？） 我们需要把事件注销掉以保证他们能够被原生事件执行 */
  ...
  /** @foxzero-007 事件机制 增加补捕获机制 passive会透出*/
  if (isCapturePhaseListener) {
